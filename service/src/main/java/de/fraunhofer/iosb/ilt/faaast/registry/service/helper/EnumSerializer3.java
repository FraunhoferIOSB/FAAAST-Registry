 ...
 * serialization will be used. Compatible with Jackson 3.
 */
@SuppressWarnings("rawtypes")
public class EnumSerializer3 extends ValueSerializer<Enum> {
    ...
    public void serialize(Enum value, JsonGenerator gen, SerializationContext ctxt) throws JacksonException {
        if (value instanceof DataTypeDefXsd) {
            ...
            ctxt.findValueSerializer(Enum.class).serialize(value, gen, ctxt);
        }
    }
    ...
    private void handleTimeRelatedValue(JsonGenerator gen, Enum<?> value) {
        String enumString = serializeEnumName(value.name());
        String adaptedEnumString = "xs:g" + enumString.substring(1, 2).toUpperCase() + enumString.substring(2);
