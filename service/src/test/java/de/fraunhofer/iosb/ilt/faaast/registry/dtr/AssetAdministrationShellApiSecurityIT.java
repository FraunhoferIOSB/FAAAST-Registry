...
import static de.fraunhofer.iosb.ilt.faaast.registry.dtr.helper.TestUtil.getEncodedValue;
import static de.fraunhofer.iosb.ilt.faaast.registry.dtr.helper.TestUtil.serialize;
import static org.hamcrest.Matchers.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

import de.fraunhofer.iosb.ilt.faaast.registry.dtr.helper.TenantFactory;
import de.fraunhofer.iosb.ilt.faaast.registry.dtr.helper.TestUtil;
...
public class AssetAdministrationShellApiSecurityIT extends AbstractAssetAdministrationShellApi {
    ...
    @Before
    public void setUp() {
        keyPrefix1 = UUID.randomUUID().toString();
        keyPrefix2 = UUID.randomUUID().toString();
    }
    ...
    public void testGetAllShellsWithDefaultClosedFilteredSpecificAssetIdsByTenantId() throws Exception {
        AssetAdministrationShellDescriptor shellPayload = TestUtil
                .createCompleteAasDescriptor(keyPrefix1 + "semanticId", "http://example.com/");
        shellPayload.setId(keyPrefix1);
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .get(createURL(SHELL_BASE_PATH))
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantOne().getTenantId())
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .get(createURL(SHELL_BASE_PATH))
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantTwo().getTenantId())
                        .queryParam("pageSize", "100")
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(MockMvcResultHandlers.print())
                .andExpect(status().isOk())
    ...
    public void testGetShellWithFilteredSpecificAssetIdsByTenantId() throws Exception {
        SpecificAssetId asset1 = TestUtil.createSpecificAssetId(keyPrefix1 + "CustomerPartId", "tenantTwoAssetIdValue",
                List.of(TenantFactory.tenantTwo().getTenantId()));
        SpecificAssetId asset2 = TestUtil.createSpecificAssetId(keyPrefix1 + "CustomerPartId2", "tenantThreeAssetIdValue",
                List.of(TenantFactory.tenantThree().getTenantId()));
        SpecificAssetId asset3 = TestUtil.createSpecificAssetId(keyPrefix1 + "MaterialNumber", "withoutTenantAssetIdValue",
                List.of(TenantFactory.tenantTwo().getTenantId()));
        // Define specificAsset with wildcard which not allowed. (Only manufacturerPartId is defined in application.yml)
        SpecificAssetId asset4 = TestUtil.createSpecificAssetId(keyPrefix1 + "BPID", "ignoreWildcard", List.of(getExternalSubjectIdWildcardPrefix()));
        // Define specificAsset with wildcard which is allowed. (Only manufacturerPartId is defined in application.yml)
        SpecificAssetId asset5 = TestUtil.createSpecificAssetId("manufacturerPartId", keyPrefix1 + "wildcardAllowed",
    ...
    public void testGetShellWithFilteredSpecificAssetIdsByTenantId(List<SpecificAssetId> specificAssetIds, List<SpecificAssetId> expectedSpecificAssetIds)
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .get(createURL(SINGLE_SHELL_BASE_PATH), encodedShellId)
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantOne().getTenantId())
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(MockMvcResultHandlers.print())
                .andExpect(status().isOk())
    ...
    //                .add(specificAssetId("CustomerPartId", tenantTwoAssetIdValue, TenantFactory.tenantTwo().getTenantId()))
    //                .add(specificAssetId("CustomerPartId", tenantThreeAssetIdValue, TenantFactory.tenantThree().getTenantId()))
    ...
    //        mvc.perform(
    //                MockMvcRequestBuilders
    //                        .post(createURL(SHELL_BASE_PATH) + "/fetch")
    ...
    public void testGetSpecificAssetIdsFilteredByTenantId() throws Exception {
        ...
        Reference externalSubjectId = new DefaultReference();
        Key key = new DefaultKey();
        key.setType(KeyTypes.SUBMODEL);
        key.setValue(TenantFactory.tenantOne().getTenantId());
        externalSubjectId.setKeys(List.of(key));
        externalSubjectId.setType(ReferenceTypes.EXTERNAL_REFERENCE);
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .post(createURL(SINGLE_LOOKUP_SHELL_BASE_PATH), getEncodedValue(shellId))
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantOne().getTenantId())
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .get(createURL(LOOKUP_SHELL_BASE_PATH))
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantOne().getTenantId())
    ...
    public void testFindExternalShellIdsBySpecificAssetIdsWithTenantBasedVisibilityAndWildcardExpectSuccess() throws Exception {
        ...
        SpecificAssetId asset1 = TestUtil.createSpecificAssetId(keyPrefix1 + "defaultClosed", "value_1", null);
        ...
        SpecificAssetId asset2 = TestUtil.createSpecificAssetId("manufacturerPartId", keyPrefix1 + "value_2",
                List.of(getExternalSubjectIdWildcardPrefix()));
        ...
        SpecificAssetId asset3 = TestUtil.createSpecificAssetId(keyPrefix1 + "bpId", "value_3", List.of(getExternalSubjectIdWildcardPrefix()));
        // asset3 is visible for tenantTwo and tenantThree
        SpecificAssetId asset4 = TestUtil.createSpecificAssetId(keyPrefix1 + "tenantTwo_tenantThree", "value_3",
                List.of(TenantFactory.tenantTwo().getTenantId(), TenantFactory.tenantThree().getTenantId()));
        // asset4 is visible for tenantTwo, because externalSubjectId = tenantTwo
        SpecificAssetId asset5 = TestUtil.createSpecificAssetId(keyPrefix1 + "tenantTwo", "value_2_private",
                List.of(TenantFactory.tenantTwo().getTenantId()));
        ...
        SpecificAssetId sa1 = TestUtil.createSpecificAssetId(keyPrefix1 + "defaultClosed", "value_1", null);
        SpecificAssetId sa2 = TestUtil.createSpecificAssetId("manufacturerPartId", keyPrefix1 + "value_2", null);
        SpecificAssetId sa3 = TestUtil.createSpecificAssetId(keyPrefix1 + "bpId", "value_3", null);
        SpecificAssetId sa4 = TestUtil.createSpecificAssetId(keyPrefix1 + "tenantTwo_tenantThree", "value_3", null);
        SpecificAssetId sa5 = TestUtil.createSpecificAssetId(keyPrefix1 + "tenantTwo", "value_2_private", null);
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .get(createURL(LOOKUP_SHELL_BASE_PATH))
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantThree().getTenantId())
                        .queryParam("assetIds", encodedSa2)
                        .queryParam("assetIds", encodedSa5)
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(MockMvcResultHandlers.print())
    ...
    public void testGetAllShellsByOwningTenantId() throws Exception {
        AssetAdministrationShellDescriptor shellPayload = TestUtil
                .createCompleteAasDescriptor(keyPrefix2 + "semanticId", "http://example.com/");
        shellPayload.setId(keyPrefix2);
        List<SpecificAssetId> shellpayloadSpecificAssetIDs = shellPayload.getSpecificAssetIds();
    ...
    public void testGetAllShellsWithPublicAccessByTenantId() throws Exception {
        ...
        // asset1 is only visible for the owner because the externalSubjectId = null
        SpecificAssetId asset1 = TestUtil.createSpecificAssetId(keyPrefix2 + "defaultClosed", "value_1", null);
        // asset2 is visible for everyone, because externalSubjectId = PUBLIC_READABLE and specificAssetKey is manufacturerPartId (which is in the list of allowedTypes via application.yml)
        SpecificAssetId asset2 = TestUtil.createSpecificAssetId("manufacturerPartId", keyPrefix2 + "value_2",
                List.of(getExternalSubjectIdWildcardPrefix()));
        // asset3 is visible for tenantTwo, because externalSubjectId = tenantTwo
        SpecificAssetId asset3 = TestUtil.createSpecificAssetId(keyPrefix2 + "tenantTwo", "value_2_public",
                List.of(TenantFactory.tenantTwo().getTenantId()));
    ...
    public void testGetShellByExternalIdByOwningTenantId() throws Exception {
        ...
        mvc.perform(
                MockMvcRequestBuilders
                        .get(createURL(SINGLE_SHELL_BASE_PATH), getEncodedValue(shellPayload.getId()))
                        .header(EXTERNAL_SUBJECT_ID_HEADER, TenantFactory.tenantOne().getTenantId())
                        .queryParam("pageSize", "100")
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(MockMvcResultHandlers.print())
